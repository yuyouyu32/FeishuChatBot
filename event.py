...
class MessageReceiveEvent(Event):
    ...
    def __init__(self, dict_data, token, encrypt_key):
        super().__init__(dict_data, token, encrypt_key)
        event = dict_data.get("event")
        self.mentions = event['message'].get('mentions', [])

    @staticmethod
    def event_type():
        return "im.message.receive_v1"

class AddGoupEvent(Event):
    # add group event defined in https://open.feishu.cn/document/server-docs/group/chat-member/event/added-2
    @staticmethod
    def event_type():
...
class EventManager(object):
    ...
    event_type_map = dict()
    _event_list = [MessageReceiveEvent, UrlVerificationEvent, AddGoupEvent]
